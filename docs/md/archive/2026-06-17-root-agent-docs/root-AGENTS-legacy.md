...
_更新：2026-03-10 · 目标分支：main · 工作目录：/Users/minxian/conductor/workspaces/toothbrush_game/riga_
...
## 13. 浏览器自动化测试（Browser Automation）

使用 `agent-browser` 进行 Web 自动化测试。运行 `agent-browser --help` 查看所有命令。

### 13.1 核心工作流程

1. **打开页面**：`agent-browser open <url>` - 导航到目标页面
2. **获取快照**：`agent-browser snapshot -i` - 获取可交互元素及其引用（@e1, @e2）
3. **交互操作**：
