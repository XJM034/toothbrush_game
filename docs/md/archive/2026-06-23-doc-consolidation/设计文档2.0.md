...
## 11. Web MVP 实际实现总结（iOS 迁移参考）

> 本节记录 Web MVP 的实际实现路径、参数配置和关键代码结构，供 iOS 原生开发参考。
...
│       ├── face_landmarker.task   # 人脸检测模型 (~11MB)
│       └── hand_landmarker.task   # 手部检测模型 (~8MB)
├── img/                           # 头套图片资源
│   ├── cat.png
│   ├── dog.png
│   └── rabbit.png
├── src/
│   ├── mediapipe/                 # MediaPipe 封装层
│   │   ├── FaceTracker.ts         # 人脸追踪器
...
│   │   │   ├── Fist.ts            # 握拳检测
│   │   │   ├── Shake.ts           # 晃动检测
│   │   │   └── BrushGesture.ts    # 综合刷牙手势检测
│   │   ├── game/
│   │   │   └── GameStateMachine.ts # 游戏状态机
│   │   ├── rendering/
│   │   │   ├── AvatarRenderer.ts  # 头套渲染器
│   │   │   └── DebugRenderer.ts   # 调试信息渲染
│   │   └── utils/
│   │       ├── geometry.ts        # 几何计算工具
│   │       └── smoothing.ts       # EMA 平滑算法
│   ├── config/
│   │   ├── mediapipe.config.ts    # MediaPipe 配置
│   │   ├── default.config.ts      # 默认游戏配置
│   │   └── avatar.config.ts       # 头套配置
│   ├── hooks/                     # React Hooks
│   │   ├── useCamera.ts           # 摄像头管理
│   │   ├── useMediaPipe.ts        # MediaPipe 检测循环
│   │   └── useGameStateMachine.ts # 游戏状态管理
│   ├── components/                # React 组件
│   │   ├── GameScreen.tsx         # 游戏主屏幕
│   │   ├── GamePlayScreen.tsx     # 游戏进行中屏幕
│   │   └── AvatarSelector.tsx     # 头套选择器
│   └── types/                     # TypeScript 类型定义
│       ├── mediapipe.types.ts     # MediaPipe 结果类型
│       ├── detector.types.ts      # 检测器结果类型
│       └── game.types.ts          # 游戏状态类型
└── package.json
```

### 11.2 MediaPipe 模型与配置

#### 使用的模型

| 模型 | 文件 | 大小 | 输出 |
|------|------|------|------|
| Face Landmarker v1 | `face_landmarker.task` | ~11MB | 468 个人脸关键点 + Blendshapes |
| Hand Landmarker v1 | `hand_landmarker.task` | ~8MB | 21 个手部关键点 + 世界坐标 |
...
// src/config/mediapipe.config.ts
{
  ...
  faceOptions: {
    ...
    outputFaceBlendshapes: true,      // 重要：输出表情系数
...
- iOS 使用 **MediaPipe Tasks Swift/ObjC SDK**
- 模型文件相同（`.task` 格式跨平台通用）
- 配置参数可直接迁移
...
┌─────────────────────────────────────────────────────────┐
│  MediaPipe 检测层                                        │
│  ├─ FaceTracker.detectForVideo()                        │
│  │   → landmarks[468], blendshapes, faceCenter,         │
│  │     faceScale, faceRotation                          │
│  └─ HandTracker.detectForVideo()                        │
│      → landmarks[21], worldLandmarks, handedness        │
└─────────────────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────────────────┐
│  手势检测层 (BrushGesture)                               │
│  ├─ TeethGate.detect() → 露牙判定                       │
│  ├─ Fist.detect() → 握拳判定                            │
│  └─ Shake.detect() → 晃动判定                           │
│  综合输出: stage, isBrushing, confidence                │
└─────────────────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────────────────┐
│  游戏状态机 (GameStateMachine)                           │
│  状态: init → ready → playing → brushing → success      │
│  输出: gameState, score, remainingTime                  │
└─────────────────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────────────────┐
│  渲染层                                                  │
...
#### 11.4.1 露牙检测 (TeethGate)

**文件**: `src/core/detectors/TeethGate.ts`

**算法**:
- 使用 `jawOpen` blendshape 分数
- 阈值判定 + 稳定性检测

**实际参数**:
```typescript
jawOpenThreshold = 0.4    // jawOpen >= 0.4 表示嘴巴张开
requiredStableFrames = 5  // 需要连续 5 帧 (~167ms @ 30fps)
```

**iOS 迁移要点**:
- MediaPipe iOS SDK 同样输出 blendshapes
- 关键 blendshape: `jawOpen`

#### 11.4.2 握拳检测 (Fist)

**文件**: `src/core/detectors/Fist.ts`

**算法**:
- 检测手指是否弯曲（指尖到指根距离 < 手腕到指根距离 × 0.4）
- 统计弯曲手指数量

**实际参数**:
```typescript
curledFingerThreshold = 3  // 至少 3 根手指弯曲即认为握拳

// 手部关键点索引 (MediaPipe Hand Landmarks)
fingerTipIndices = [4, 8, 12, 16, 20]   // 各手指顶端
fingerBaseIndices = [2, 6, 10, 14, 18]  // 各手指基部
wristIndex = 0                           // 手腕

// 弯曲判定
fingerLength < wristToBase * 0.4  // 手指长度 < 手腕到基部距离的 40%
```
...
**文件**: `src/core/detectors/Shake.ts`

**算法**:
- 追踪手掌中心 (landmark 9) 的运动轨迹
- 计算归一化速度
- 统计高速帧占比

**实际参数**:
```typescript
speedThreshold = 0.02      // 速度阈值（相对于 √(width×height)）
windowMs = 500             // 滑窗时长 500ms
highSpeedRatio = 0.15      // 需要 15% 的帧超过速度阈值
requiredStableFrames = 4   // 需要连续 4 帧 (~133ms @ 30fps)
maxHistorySize = 15        // 保存最近 15 帧数据

// 速度计算
speed = pixelDistance / √(canvasWidth × canvasHeight)
```

**iOS 迁移要点**:
- 使用 world coordinates 可获得更稳定的速度计算
- 速度阈值可能需要根据实际设备调整

#### 11.4.4 综合刷牙手势 (BrushGesture)

**文件**: `src/core/detectors/BrushGesture.ts`

**状态流转**:
```
waiting → teeth_open → fist_ready → brushing → complete
                ↑                                  │
                └──────────────────────────────────┘
                        (可连续得分)
```
...
```typescript
// 露牙锁定机制
teethLockTimeout = 3000    // 3秒没露牙则解锁
teethConfirmed = true/false // 锁定状态

// 刷牙动作判定
minBrushingDuration = 800  // 需要持续握拳+晃动 800ms
...
**文件**: `src/core/game/GameStateMachine.ts`
...
**状态转换**:
```
init ──自动──> ready ──露牙──> playing ──握拳+晃动──> brushing
                                  ↑                      │
                                  │                      │ (800ms)
                                  │                      ↓
                              playing <──500ms── success
                                  │
                                  │ (3秒没露牙)
                                  ↓
                                ready
...
**计分规则**:
```typescript
scorePerBrush = 10        // 每次刷牙基础分
bonusForAccuracy = 5      // 准确率奖励（× confidence）
gameDuration = 60000      // 游戏时长 60 秒
```

### 11.6 头套渲染 (Face Filter)

**文件**: `src/core/rendering/AvatarRenderer.ts`

**定位算法**:
```typescript
// 1. 获取人脸边界信息
faceBounds = getFaceBoundsFromLandmarks(landmarks, width, height)
// 返回: center, forehead, chin, eyeDistance, faceWidth, faceHeight, rotation
...
```typescript
leftEyeIdx = 33      // 左眼外侧
rightEyeIdx = 263    // 右眼外侧
foreheadIdx = 10     // 额头中心
chinIdx = 152        // 下巴
leftFaceIdx = 234    // 左脸边缘
rightFaceIdx = 454   // 右脸边缘
```
...
```typescript
{
  ...
  name: '🐶 小狗',
  imgUrl: '/img/dog.png',
  faceHoleOffset: { x: 0, y: 0.25 },  // 脸洞在图片下方 25%
  anchorOffset: { x: 0, y: -0.15 },   // 整体向上移动 15%
  scale: 1.0
}
```
...
**EMA 平滑参数**:
```typescript
smoothingAlpha = 0.3  // 平滑系数（越小越平滑，越大越灵敏）
...
#### 可直接复用的部分
- [ ] MediaPipe 模型文件 (`.task` 格式)
- [ ] 手势检测算法逻辑（TeethGate, Fist, Shake）
- [ ] 所有阈值参数
- [ ] 游戏状态机逻辑
- [ ] 头套定位算法
- [ ] EMA 平滑算法
...
#### 需要平台适配的部分
- [ ] 摄像头帧获取 (AVCaptureSession)
- [ ] MediaPipe SDK 初始化 (Swift/ObjC API)
- [ ] 渲染层 (Core Graphics / Metal)
- [ ] UI 框架 (SwiftUI / UIKit)

#### iOS MediaPipe 集成参考
```swift
// 示例：iOS MediaPipe Tasks 初始化
...
| 指标 | Web MVP 实测值 | iOS 建议目标 |
|------|---------------|-------------|
| 检测 FPS | 20-30 | 30 |
| 渲染 FPS | 30 | 60 |
| 摄像头分辨率 | 640×480 | 640×480 |
| Face 检测延迟 | ~30ms | <20ms |
| Hand 检测延迟 | ~25ms | <15ms |
| 内存占用 | ~200MB | <150MB |
