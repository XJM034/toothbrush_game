...
## 5.X 卡通头套自动跟随头部（Face Filter / AR Hat Overlay）（新增）

### 5.X.1 是否能用 MediaPipe 实现（结论）

可以。**MediaPipe Tasks Vision 的 Face Landmarker**会输出：

* **faceLandmarks（人脸关键点）**：用于计算头套位置/大小/旋转
* **facialTransformationMatrixes（人脸 4x4 变换矩阵，可选开启）**：用于更稳定的“贴合头部姿态”的效果（转头、抬头、歪头更稳）([Google AI for Developers][1])

> 你这个刷牙场景（正对镜头、距离/角度相对固定）属于低~中等难度，Web MVP 完全可落地。

### 5.X.2 推荐实现路径（Web MVP：2D PNG 头套）

采用 **video + canvas 叠加**：

* `<video>`：播放摄像头流
* `<canvas>`：绘制透明背景的头套 PNG（或一组 PNG 动画帧）
* 每帧用 Face Landmarker 的结果更新：`x / y / scale / rotation`

Face Landmarker 在视频流场景使用 `detectForVideo()`；注意该调用**同步阻塞主线程**，如出现掉帧可迁移到 Web Worker（你文档里已有 Worker 章节建议保留）。([Google AI for Developers][2])

### 5.X.3 头套贴合的关键计算（够用且稳定）

**输入**：`faceLandmarks`（归一化坐标，0~1）([Google AI for Developers][1])
**输出**：Canvas 上的仿射变换参数
...
1. **锚点（位置）**
   ...
   * 取双眼附近关键点的中心（或鼻梁/额头附近点）作为基准点，再向上做一个固定比例偏移（落在额头位置）
2. **缩放（尺寸）**

   * 用两眼距离（或脸宽）作为尺度基准，乘一个经验系数（不同头套可配置不同系数）
3. **旋转（roll）**

   * 用双眼连线的角度 `atan2(dy, dx)` 得到“歪头角”，用于头套旋转

> 进阶：开启 `outputFacialTransformationMatrixes` 后，你可以用矩阵做更接近 AR 的姿态贴合（yaw/pitch/roll 都能更稳）。([Google AI for Developers][1])

### 5.X.4 必须处理的 3 个坑（否则效果会“看起来不像你图里那样稳”）

1. **镜像问题（前置摄像头常见）**

   * 浏览器预览往往是“镜像显示”，但检测/绘制坐标要统一：
   * 规则：**要么 video+canvas 都镜像，要么都不镜像**，否则头套会左右反、漂移。
2. **抖动（landmark 天然有微抖）**

   * 对 `x/y/scale/rotation` 做 EMA 平滑（低成本、立竿见影）
   * 丢脸 N 帧：头套渐隐并提示“请对准镜头”
3. **iOS 上视频内联播放（影响 canvas 叠加体验）**

   * iPhone 上建议给 `<video>` 加 `playsinline`，避免自动全屏导致叠加层失效/交互断裂。([WebKit][3])

### 5.X.5 运行环境要求（补齐到“环境/部署”章节）

* 摄像头 `getUserMedia()` **仅在安全上下文可用**（HTTPS 或 localhost），否则会报权限/安全错误。([MDN Web Docs][4])
* 兼容性策略：最低保障最新 Chrome/Edge/Safari；低端机可降采样推理帧率（每 2~3 帧跑一次 detectForVideo）。

### 5.X.6 后续可选增强（不影响 MVP）

* 头套从 2D PNG → 3D（Three.js/WebGL），用 transformation matrix 做更真实的空间贴合
* 遮挡（头发/手在前）：需要额外分割/深度能力，MVP 不做

---

如果你把这段加进文档后，**在“模块划分/数据流”里再补一句**就闭环了：

> FaceTracker 输出不仅用于 TeethGate，也用于 **AvatarOverlayRenderer（头套渲染器）**，该渲染器在游戏全过程常驻。

你要是愿意，我也可以顺手把这段按你文档现有目录位置（比如放在“核心算法设计”或“渲染层”下）给你**标注具体插入点**，让整份文档结构更顺。

[1]: https://ai.google.dev/edge/mediapipe/api/solutions/js/tasks-vision.facelandmarkerresult?utm_source=chatgpt.com "FaceLandmarkerResult interface  |  Google AI Edge  |  Google AI for Developers"
[2]: https://ai.google.dev/edge/mediapipe/api/solutions/js/tasks-vision.facelandmarker?utm_source=chatgpt.com "FaceLandmarker class  |  Google AI Edge  |  Google AI for Developers"
[3]: https://webkit.org/blog/6784/new-video-policies-for-ios/?utm_source=chatgpt.com "New <video> Policies for iOS | WebKit"
[4]: https://developer.mozilla.org/docs/Web/API/MediaDevices/getUserMedia?utm_source=chatgpt.com "MediaDevices: getUserMedia() method - Web APIs | MDN"
